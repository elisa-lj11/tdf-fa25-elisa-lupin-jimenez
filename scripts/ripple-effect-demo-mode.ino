...
// ---------- Demo Mode Globals ----------
String activeLocation; // Will be set in setup
const unsigned long demoInterval = 7 * 1000; // 15 seconds
unsigned long lastDemoSwitch = 0;
int currentLocationIndex = 0; // To iterate through the locations
...
void setup() {
  ...
  Serial.println("Location names loaded for demo mode.");
  ...
  activeLocation = locationNames[currentLocationIndex];
  // --- End Demo Mode Setup ---
  ...
  lastUpdate = millis(); // We can use this to track the last fetch
  lastDemoSwitch = millis(); // Reset the 15-sec *demo switch* timer
}
...
void loop() {
  ...
  if (millis() - lastDemoSwitch > demoInterval) {
    
    // Increment index and loop back to 0 if at the end
    currentLocationIndex++;
    if (currentLocationIndex >= locationNames.size()) {
      currentLocationIndex = 0;
    }
    
    // Get the new active location name
    activeLocation = locationNames[currentLocationIndex];
    ...
    // We just fetched data, so we can also reset the lastUpdate timer
    lastUpdate = millis(); 
  }
  // --- End Demo Mode Logic ---
