/*
Ripple-effect-v3
Updating backdrop colors based on waveHeight.
Vibe-coded with Gemini: https://gemini.google.com/share/bca992c6a043
*/
...
/*const double latitude  = 39.356486;
const double longitude = -122.026527;*/
...
void visualizeRipple(int dir, double period, double height) {
  ...
  // We'll map height from 0.0 (Purple) to 5.0 (Red)
  double constrainedHeight = constrain(height, 0.0, 5.0);
  double fraction = constrainedHeight / 5.0; // 0.0 (low) to 1.0 (high)

  // Lerp from a dim Purple (30, 0, 40) to a dim Red (51, 0, 0)
  // Full brightness vals: Purple (150, 0, 200) -> Red (255, 0, 0)
  // We use dim values (approx 20%) so the ripples stand out.
  int final_red = (int)(30 + (51 - 30) * fraction);
  int final_green = 0; // Always 0 for Red <-> Purple
  int final_blue = (int)(40 + (0 - 40) * fraction);
  
  uint16_t backdropColor = matrix.Color(final_red, final_green, final_blue);
